...
> - **换 key 别只改文件就以为完事**——如果你以前用 `/plugin configure` 配过 key，那值存 Keychain、**跨卸载/重装不清**。现在文件优先级高于 userConfig，改文件能用；但两者不一致时日志会一直飘 WARNING。想彻底干净：`/plugin configure agent-bark-notify@agent-bark-notify` 把 bark_key 清空。（Bark 服务器对错 key 都回 `200 success`，用错 key 时日志全绿、手机却收不到——见下文 FAQ。）
...
**Q：装完没收到通知？**
按顺序排查：① `/plugin` 菜单确认 plugin 是 **enabled**（install 不自动 enable）；② **重启 Claude Code 会话**（hook 启动时加载）；③ 确认 `~/.config/agent-bark-notify/bark.key` 在；④ 看 `/tmp/agent-bark-notify.log` 最近一行 `send: key=XXXX***`，前 4 位对得上你当前的 key 才算用对；⑤ 若日志里有 `WARN: bark.key 文件与 plugin userConfig 的 key 不一致`，说明以前 `/plugin configure` 配过旧 key，跑 `/plugin configure agent-bark-notify@agent-bark-notify` 清掉；⑥ 手动测链路：
```bash
...
```
手动能收到 = 脚本和 key 都通，问题在 hook 没触发（回看 ①②）；日志里 key 前缀不对 = 用错 key（看 ④⑤）。**Bark 对错 key 都回 `200 success`，日志全绿 ≠ 手机能收到——key 前缀是唯一不会骗人的判据。**
